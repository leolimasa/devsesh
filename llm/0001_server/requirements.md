...
* `/api/v1/auth/pair/exchange` [req.p2xw1a]
	* Called by the authenticated web client to approve a pending pairing code. Requires a valid JWT in the `Authorization` header. [req.n3hk7b]
	* Validates that the code exists, has not been used, and has not expired, then marks it as approved. [req.q8mv5c]
	* Returns `200 OK` with no token — the CLI polls `pair/complete` to obtain the token once approved. [req.r7ft2d]
* `/api/v1/auth/pair/complete` [req.hq0gcy]
	...
	* The code must have been approved via `pair/exchange` before this endpoint will issue a token. [req.w5yn3x]
