...
* Create an integration test that validates that the Session Detail Page terminal (xterm): [req.sry715]
	* Is able to connect via SSH to an existing tmux session with the same ID as the session ID [req.gy4af9]
	* Is able to type commands [req.vqjj4e]
	* Is able to receive output from the remote host [req.ow4f94]
* Fix the Session Detail Page terminal so that it is able to read the remote tmux screen [req.gpbadi]
* Fix the Session Detail Page terminal so that it is able to send keystrokes / input to the remote tmux screen [req.68mx5a]
